...
class RotatedYOLOXHead(BaseDenseHead):
    ...
    def predict_by_feat(self, 
        ...
        mlvl_strides = [
            ...
            featmap_size, stride in zip(featmap_sizes, self.strides)
        ]
        ...
        for img_id, img_meta in enumerate(batch_img_metas):
            ...
            results = InstanceData(
                bboxes=RotatedBoxes(flatten_decoded_bboxes[img_id][valid_mask]),
    ...
    def loss_by_feat(self, cls_scores, bbox_preds, angle_preds,objectnesses, batch_gt_instances, batch_img_metas, batch_gt_instances_ignore=None, **kwargs):
        num_imgs = len(batch_img_metas)
        ...
        flatten_tblr_preds = torch.cat(flatten_tblr_preds, dim=1)
        flatten_tblr_preds = flatten_tblr_preds * flatten_priors[..., -1, None]
        flatten_angle_preds = torch.cat(flatten_angle_preds, dim=1)
        flatten_decoded_angle = self.angle_coder.decode(
            flatten_angle_preds, keepdim=True)
        flatten_tblra = torch.cat([flatten_tblr_preds, flatten_decoded_angle],
                                  dim=-1)
        flatten_rbboxes = distance2obb(flatten_priors[..., :2], flatten_tblra,angle_version=self.angle_version)
        # flatten_bbox_preds = torch.cat([flatten_bbox_preds, flatten_angle_preds], dim=2)
        # flatten_bbox_preds = flatten_bbox_preds.view(num_imgs, -1, 5)  # (dx, dy, dw, dh, dt)
    ...
    def _get_targets_single(
        ...
        num_gts = len(img_meta)
        
        gt_info = gt_instances_preprocess(gt_instances, num_gts)
        gt_labels = gt_info[:, :, :1]
        gt_bboxes = gt_info[:, :, 1:]  # xywha
        pad_bbox_flag = (gt_bboxes.sum(-1, keepdim=True) > 0).float()
        ...
        assigned_result = self.assigner(decoded_bboxex.detach(),
                                        cls_preds.detach(),
                                        offset_priors, gt_labels,
                                        gt_bboxes, pad_bbox_flag)
        sampling_result = self.sampler.sample(assigned_result, pred_instances, gt_instances)
        pos_inds = sampling_result.pos_inds
        num_pos_per_img = pos_inds.size(0)
        
        pos_ious = assigned_result.max_overlaps[pos_inds]
